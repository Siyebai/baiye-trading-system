...
# -*- coding: utf-8 -*-
"""
白夜纸交易 v6.7 — 修复版（1m高频，实时数据）
核心修复:
  - cc必须在方向切换时重置（Bug修复）
  - cc只统计当前连涨/连跌累计，不跨越方向
  - 增加实时调试输出，每轮输出当前指标状态
  
目标: 今天完成≥10笔完整闭环
"""
import json, time, requests, numpy as np, pandas as pd, warnings, sys
...
LOG_FILE    = Path("logs/paper_v67.log")
STATE_FILE  = Path("logs/paper_v67_state.json")
TRADES_FILE = Path("logs/paper_v67_trades.json")
...
logger = logging.getLogger('paper_v67')
...
def compute(df):
    ...
    hc = np.abs(df['high'].values - prev_close)
    lc_ = np.abs(df['low'].values - prev_close)
    tr = np.maximum(hl, np.maximum(hc, lc_))
    df['atr'] = _wilder(tr, 14)

    pm = np.diff(df['high'].values, prepend=df['high'].values[0])
    nm = -np.diff(df['low'].values, prepend=df['low'].values[0])
    pm = np.clip(pm, 0, None)
    nm = np.clip(nm, 0, None)
    ...
    # ✅ 修复: cc在方向切换时重置（只累计当前连涨/连跌）
    closes = df['close'].values
    cu = np.zeros(len(df), dtype=int)
    cd = np.zeros(len(df), dtype=int)
    cc = np.zeros(len(df), dtype=float)  # 当前方向累计涨跌幅
...
def main():
    ...
    logger.info("="*70)
    logger.info("白夜纸交易 v6.7 — 1m高频 Bug修复版 | 币安实时数据")
    logger.info(f"品种: {SYMBOLS} | 时间框架: {INTERVAL}")
    logger.info(f"资金={CAPITAL}U 风险={RISK_PCT*100}% FEE={FEE*100:.3f}%/side")
    logger.info(f"目标: ≥{TARGET_TRADES}笔闭环 | 最多{MAX_POSITIONS}个持仓")
    logger.info("="*70)
    logger.info(f"启动状态: 权益={state['equity']:.2f}U 持仓={list(state['positions'].keys())}")

    cooldown = {}; last_scan = {}; poll_count = 0
    ...
    while True:
        try:
            ...
            if state['day_loss'] >= state['equity'] * DAILY_LOSS_PCT:
                logger.warning(f"⛔ 日熔断! day_loss={state['day_loss']:.2f}U 停止开仓")
            ...
            if open_cnt < MAX_POSITIONS:
                for sym in SYMBOLS:
                    ...
                    try:
                        ...
                        logger.warning(f"fetch {sym} err: {e}"); continue
                    bar_ts = int(df.iloc[-2]['ts'])
                    if last_scan.get(sym) == bar_ts: continue
            ...
            if poll_count % 20 == 0:
                ...
                logger.info(
                    f"📊 [{now.strftime('%H:%M')}] "
                    f"已完成={total}/{TARGET_TRADES}笔 WR={wr:.1f}% "
                    f"累计PnL={pnl:+.3f}U 权益={state['equity']:.2f}U | "
                    f"持仓={open_pos if open_pos else '无'}"
                )
            
            # 每轮输出关键指标（调试模式，每3轮）
            if poll_count % 3 == 0 and not state['positions']:
                try:
                    # 快速检查最活跃品种
                    for sym in ['LINKUSDT','BNBUSDT']:
                        r = requests.get("https://api.binance.com/api/v3/klines",
                            params={'symbol':sym,'interval':INTERVAL,'limit':50}, timeout=5)
                        df = pd.DataFrame(r.json(), columns=['ts','open','high','low','close','vol','ct','qv','tr','tbb','tbq','ign'])
                        for c in ['open','high','low','close']: df[c]=df[c].astype(float)
                        df = compute(df)
                        row = df.iloc[-2]
                        ...
                        cfg = CONFIGS[sym]
                        dist_s = f"cu{cu}/{cfg['sc']} cc{cc:.3f}%/{cfg['ccp']*100:.3f}%"
                        dist_l = f"cd{cd}/{cfg['lc']} cc{cc:.3f}%/{-cfg['ccp']*100:.3f}%"
                        logger.info(f"  📡 {sym}: ADX={adx:.1f} ATR={atr:.5f} | SHORT距离=[{dist_s}] LONG距离=[{dist_l}]")
                except: pass
