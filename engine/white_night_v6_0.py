...
增强层：多重过滤机制（成交量/RSI极端/MACD确认/动态TP）
...
class Params:
    ...
    PARAMS_5M = {
        ...
        "BNBUSDT": {"sc": 7, "lc": 3, "ccp": 0.0008, "adx_th": 25,
                     "tp_s": 0.8, "tp_l": 0.7, "sl_atr": 1.0, "long_disabled": True},
    }
...
class WhiteNight:
    ...
    def run_full_test(self) -> None:
        ...
        if valid:
            ...
            print(f"  15m基线 — 平均胜率: {avg_wr:.1f}%, 平均PF: {avg_pf:.2f}, "
                  f"平均月均: {avg_monthly:.1f}%")
            print(f"  胜率≥55%品种: {above_55}/{len(valid)}")

            # 对比原始引擎基线
            print(f"\n  📊 v5.7 vs v6.0 vs 原始引擎 对比:")
            print(f"    v5.7 (EMA+DI+RSI):  平均胜率≈24.5%, PF≈0.20  ❌ 完全失效")
            print(f"    v6.0 (连涨连跌核心): 平均胜率≈{avg_wr:.1f}%, PF≈{avg_pf:.2f}  {'✅' if avg_wr >= 55 else '⚠️'}")
