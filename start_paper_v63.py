...
# -*- coding: utf-8 -*-
"""
白夜纸交易 v6.3 — 100笔闭环验证版
核心改动（vs v6.2）：
  1. ADX阈值全部降低(更贴合横盘市场)：BTC:22→15, ETH:20→13, SOL:28→18, 等
  2. 连续K线数降低1-2根（更快触发）
  3. MAX_POSITIONS: 3→4 (更多并发)
  4. POLL_SECS: 60→30 (更快响应)
  5. 累计变化幅度CCP适度放宽
  6. 目标: 快速积累100笔完整闭环交易
"""
...
MAX_POSITIONS = 4
POLL_SECS     = 30
LOG_FILE      = Path("logs/paper_v63.log")
STATE_FILE    = Path("logs/paper_v63_state.json")
TRADES_FILE   = Path("paper_trades_v63.json")
TARGET_TRADES = 100
LOG_FILE.parent.mkdir(exist_ok=True)

_h = RotatingFileHandler(LOG_FILE, maxBytes=5*1024*1024, backupCount=2, encoding='utf-8')
_h.setFormatter(logging.Formatter('%(asctime)s %(message)s'))
logger = logging.getLogger('paper_v63')
logger.addHandler(_h)
logger.addHandler(logging.StreamHandler())
logger.setLevel(logging.INFO)

# v6.3 参数 — ADX阈值降低，快速积累信号
CONFIGS = {
    "BTCUSDT":  dict(sc=6, lc=4, ccp=0.0015, adx_th=15, tp_s=0.8, tp_l=0.7, sl_atr=1.2, long_disabled=False),
    "ETHUSDT":  dict(sc=5, lc=4, ccp=0.0012, adx_th=13, tp_s=0.8, tp_l=0.7, sl_atr=1.2, long_disabled=False),
    "SOLUSDT":  dict(sc=5, lc=4, ccp=0.0012, adx_th=18, tp_s=0.8, tp_l=0.7, sl_atr=1.2, long_disabled=False),
    "BNBUSDT":  dict(sc=5, lc=5, ccp=0.0008, adx_th=12, tp_s=0.8, tp_l=0.8, sl_atr=1.2, long_disabled=True),
    "LINKUSDT": dict(sc=5, lc=3, ccp=0.0012, adx_th=15, tp_s=0.9, tp_l=0.8, sl_atr=1.2, long_disabled=False),
    "POLUSDT":  dict(sc=5, lc=4, ccp=0.0015, adx_th=18, tp_s=0.8, tp_l=0.7, sl_atr=1.2, long_disabled=True),
    "LTCUSDT":  dict(sc=4, lc=4, ccp=0.0012, adx_th=15, tp_s=0.8, tp_l=0.7, sl_atr=1.2, long_disabled=False),
    "AVAXUSDT": dict(sc=4, lc=4, ccp=0.0008, adx_th=13, tp_s=0.8, tp_l=0.7, sl_atr=1.2, long_disabled=False),
}
...
def main():
    logger.info("="*60)
    logger.info("白夜纸交易 v6.3 — 100笔闭环验证")
    logger.info(f"品种: {SYMBOLS}")
    logger.info(f"资金: {CAPITAL}U | 风险: {RISK_PCT*100}% | 最大持仓: {MAX_POSITIONS}")
    logger.info("="*60)
    ...
    while True:
        try:
            ...
            if closed_count >= TARGET_TRADES:
                logger.info(f"🎯 已完成{TARGET_TRADES}笔！最终: {summary(trades)}")
                logger.info("纸交易目标达成，程序退出")
            ...
            for sym in list(state['positions'].keys()):
                ...
                try:
                    ...
                    if hit:
                        ...
                        icon='✅' if hit=='止盈' else '❌'
                        logger.info(f"{icon}#{len([t for t in trades if t.get('status')=='closed'])} "
                                    f"{sym} {pos['dir']} {hit} | in={entry:.4f} out={exit_price:.4f} pnl={pnl:+.4f}U")
            ...
            if len(state['positions']) < MAX_POSITIONS:
                for sym in SYMBOLS:
                    ...
                    try:
                        ...
                        pos = dict(no=trade_no, sym=sym, dir=direction, entry=entry, sl=sl, tp=tp,
                        ...
                        logger.info(f"🔔 #{trade_no} 开仓 {sym} {direction} | "
                                    ...
                                    f"ADX={adx_val:.1f} notional={notional:.1f}U")
                    except Exception as e:
