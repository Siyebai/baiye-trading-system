...
整合融合历史:
  ...
  v7.2  Kelly仓位 | WinRate Guard | 追踪止损 | 相关性控制
        多时间周期(3m/5m/15m/60m) | 信号评分 | EMA200方向过滤
...
def fetch_multi_tf(symbol: str) -> Dict[str, Optional[pd.DataFrame]]:
    """并发拉取该品种所有周期K线（ThreadPool加速），单品种异常不影响其他品种"""
    result: Dict[str, Optional[pd.DataFrame]] = {}

    def _fetch_one(tf: str):
        try:
            return tf, compute_indicators(fetch_klines(symbol, tf))
        except Exception as e:
            logger.warning(f"[{symbol}/{tf}] 数据异常: {e}")
            return tf, None

    with ThreadPoolExecutor(max_workers=len(cfg.TIMEFRAMES)) as ex:
        futures = {ex.submit(_fetch_one, tf): tf for tf in cfg.TIMEFRAMES}
...
def main() -> None:
    ...
    while _running:
        try:
            ...
            # ── 1. 并发拉取所有品种多周期K线（v7.3升级：并发内外双层）
            all_tf_data: Dict[str, Dict[str, Optional[pd.DataFrame]]] = {}
            with ThreadPoolExecutor(max_workers=min(len(cfg.SYMBOLS), 8)) as pool:
                sym_futures = {pool.submit(fetch_multi_tf, sym): sym
                               for sym in cfg.SYMBOLS}
                for fut in as_completed(sym_futures):
                    sym = sym_futures[fut]
                    try:
                        all_tf_data[sym] = fut.result()
                    except Exception as e:
                        logger.warning(f"[{sym}] 并发拉取失败: {e}")
                        all_tf_data[sym] = {}
            ...
            if len(state["positions"]) < cfg.MAX_OPEN_POSITIONS:
                ...
                for sym in cfg.SYMBOLS:
                    ...
                    for tf in [cfg.TF_PRIMARY, cfg.TF_CONFIRM, cfg.TF_FAST]:  # 15m/5m/3m 正确顺序
                        # 冷却期：同品种同tf
                        ck = f"{sym}_{tf}"
