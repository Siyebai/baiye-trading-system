...
品种配置 (v7.0整合最优参数):
  ...
  ETHUSDT   |  6 |  4 | 0.0008 |  15 | 0.8x | v6.5高频最优
  SOLUSDT   |  6 |  4 | 0.0008 |  12 | 0.8x | v6.5高频最优
  LINKUSDT  |  4 |  4 | 0.0008 |  15 | 1.2x | v6.5 TP升级
  DOTUSDT   |  4 |  5 | 0.0008 |  20 | 1.2x | v6.5 TP升级
  BNBUSDT   |  5 |  6 | 0.0015 |  15 | 0.8x | v6.9 LONG禁
  BTCUSDT   |  4 |  5 | 0.002  |  22 | 0.8x | v6.9 MEMORY
  ADAUSDT   |  4 |  5 | 0.0008 |  18 | 0.8x | v6.5高信号
  SUIUSDT   |  5 |  5 | 0.002  |  15 | 0.8x | v6.5验证
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
import json
import time
...
LOG_FILE    = WORK_DIR / "logs" / "paper_v70.log"
STATE_FILE  = WORK_DIR / "logs" / "paper_v70_state.json"
TRADES_FILE = WORK_DIR / "logs" / "paper_v70_trades.json"
PID_FILE    = WORK_DIR / "logs" / "paper_v70.pid"
...
# ══════════════════════════════════════════════════════════════
CONFIGS: dict = {
    # v6.5高频最优: WR=69%, EV=+0.442/笔
    "ETHUSDT": dict(
        sc=6, lc=4, ccp=0.0008, adx_th=15,
        tp_s=0.8, sl_atr=1.0, long_disabled=False
    ),
    # v6.5高频最优: WR=67.5%, EV=+0.363/笔
    "SOLUSDT": dict(
        sc=6, lc=4, ccp=0.0008, adx_th=12,
        tp_s=0.8, sl_atr=1.0, long_disabled=False
    ),
    # v6.5 TP升级1.2x: EV=+0.508/笔
    "LINKUSDT": dict(
        sc=4, lc=4, ccp=0.0008, adx_th=15,
        tp_s=1.2, sl_atr=1.0, long_disabled=False
    ),
    # v6.5 TP升级1.2x: EV=+0.670/笔 (最高)
    "DOTUSDT": dict(
        sc=4, lc=5, ccp=0.0008, adx_th=20,
        tp_s=1.2, sl_atr=1.0, long_disabled=False
    ),
    # v6.9 MEMORY参数: WR=63%, OOS=64.6%
    "BNBUSDT": dict(
        sc=5, lc=6, ccp=0.0015, adx_th=15,
        tp_s=0.8, sl_atr=1.0, long_disabled=True   # ← 禁LONG
    ),
    # v6.9 MEMORY参数: WR=60.4%, OOS=57.8%
    "BTCUSDT": dict(
        sc=4, lc=5, ccp=0.002, adx_th=22,
        tp_s=0.8, sl_atr=1.0, long_disabled=False
    ),
    # v6.5高信号量验证: WR=66%, EV=+0.323/笔
    "ADAUSDT": dict(
        sc=4, lc=5, ccp=0.0008, adx_th=18,
        tp_s=0.8, sl_atr=1.0, long_disabled=False
    ),
    # v6.5验证: WR=65.5%, EV=+0.309/笔
    "SUIUSDT": dict(
        sc=5, lc=5, ccp=0.002, adx_th=15,
        tp_s=0.8, sl_atr=1.0, long_disabled=False
    ),
}
...
def _setup_logger() -> logging.Logger:
    lg = logging.getLogger("paper_v70")
