...
FUSION_CONFIGS = {
    ...
    "SOLUSDT":  dict(sc=5, lc=4, ccp=0.0015, adx_th=30, filt="rsi_confirm",  tp_s=0.8, tp_l=0.8, long_disabled=False),  # v2.2: adx_th 25->30 (fix Q1 loss)
...
def check_exits(state, prices, kline_high_low=None):
    """检查持仓是否触及TP/SL
    优先用K线high/low（更准确），回退到当前价格
    kline_high_low: {sym: (high, low)} 最新已完成K线的高低点
    """
    to_close = []
    for sym, pos in list(state["positions"].items()):
        ...
        tp = pos["tp"]; sl = pos["sl"]
        # 用K线高低点判断（如果有）
        if kline_high_low and sym in kline_high_low:
            bar_high, bar_low = kline_high_low[sym]
        else:
